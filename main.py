...
def main():
    ...
    ap.add_argument("--N", type=int, default=30, help="Population size")
    ap.add_argument("--IPC", type=int, default=50, help="Images Per Class")
    ap.add_argument("--G", type=int, default=30, help="Generations")
    ap.add_argument("--steps", type=int, default=None, help="Training steps per model (overrides config)")
    ...
    ap.add_argument("--out_dir", type=str, default=None, help="Output directory (default: logs/ipc{IPC})")
    ...
    if args.out_dir is None:
        args.out_dir = f"logs/ipc{args.IPC}"
    ...
    logger = ExperimentLogger(log_dir=args.out_dir, filename=f"log_seed{args.seed}.csv")
    ...
    test_loader = torch.utils.data.DataLoader(
        test_norm, batch_size=256, shuffle=False, num_workers=4
    )
    ...
    for gen in range(args.G):
        ...
        with torch.no_grad():
            ...
            vutils.save_image(
                imgs_vis, os.path.join(args.out_dir, f"best_img_gen_{gen}.png"),
                nrow=8, padding=2,
            )

        # Save best latent
        torch.save(
            {"z": best_ind.z.cpu(), "labels": best_ind.labels.cpu()},
            os.path.join(args.out_dir, "best_z.pt"),
        )
